...
export class AwsS3Provider {
  ...
  static async getClient(region?: string) {
    const [Region, AccessId, AccessKey] = await Promise.all([
      (async () =>
        (await Env.get("AWS_S3_REGION", true)) ??
          await Env.get("AWS_REGION"))(),
      (async () =>
        (await Env.get("AWS_S3_ACCESS_ID", true)) ??
          await Env.get("AWS_ACCESS_ID"))(),
      (async () =>
        (await Env.get("AWS_S3_ACCESS_KEY", true)) ??
          await Env.get("AWS_ACCESS_KEY"))(),
    ]);
