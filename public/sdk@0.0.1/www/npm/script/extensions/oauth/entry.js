...
const securityGuard_js_1 = require("./lib/securityGuard.js");
class oauthEntry {
    ...
    static async onLogin() {
        if (!this._authInterceptorAdded) {
            index_js_1.EpicSDK._axios.interceptors.request.use(async (config) => {
                if (!config.headers["Authorization"] && this.auth) {
                    ...
                    config.headers["X-Api-Version"] = index_js_1.EpicSDK._apiVersion;
    ...
    static async registerPermissions() {
        const { scopePipeline } = await index_js_1.EpicSDK.oauthPolicies.me().data;
        this.guard = new securityGuard_js_1.SecurityGuard().load({
            scopePipeline: scopePipeline.map(($) => new Set($)),
        });
        index_js_1.EpicSDK.isPermitted = (scope, permission) => this.guard.isPermitted((typeof scope === "function" ? scope.__permission : scope) ?? "", permission);
    }
    static oauth2Login(appId, opts) {
        ...
        const url = new URL(`/oauth/login
            ...
            &lng=${opts.lng ?? "en"}
            ${opts.username ? `&username=${opts.username}` : ""}`.replace(/\s*/g, ""), index_js_1.EpicSDK._options?.axiosConfig?.baseURL).toString();
        return {
            verifier,
            url,
        };
    ...
    static async login(appId, opts) {
        const authorization = await index_js_1.EpicSDK.getCache("authorization");
        if (authorization) {
        ...
        exchangeCode: if (typeof opts?.code === "string") {
            const verifier = await index_js_1.EpicSDK.getCache("verifier");
            if (!verifier)
                break exchangeCode;
            const authorization = await this.fetchAccessToken(opts.code, verifier.value);
            await index_js_1.EpicSDK.setCache("authorization", authorization);
            return authorization;
        }
        const { verifier, url } = this.oauth2Login(appId, opts);
        await index_js_1.EpicSDK.setCache("verifier", verifier);
        opts.onRedirect(url);
    }
    static async fetchAccessToken(code, verifier, opts) {
        this.auth = await index_js_1.EpicSDK.oauth.exchangeCode({
            body: {
                code,
    ...
    static async refreshAccessToken(refreshToken) {
        //! this._refreshRequest is used to stop bubbling do not remove it!
        this.auth = await (this._refreshRequest ??= index_js_1.EpicSDK.oauth.refresh({
            body: {
                refreshToken,
    ...
    static async switchAccount(accountId) {
        ...
        await Promise.all([
            ...
            index_js_1.EpicSDK.setCache("selectedAccount", accountId),
        ]);
    }
    static async logout(allDevices = false, fcmDeviceToken) {
        await Promise.all([
            index_js_1.EpicSDK.delCache("authorization"),
            index_js_1.EpicSDK.delCache("verifier"),
            index_js_1.EpicSDK.delCache("selectedAccount"),
        ]);
        await index_js_1.EpicSDK.oauth.logout({ query: { allDevices, fcmDeviceToken } })
            .raw;
    }
}
